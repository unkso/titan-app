...
) -> db::TitanQueryResult<Vec<models::UserFileEntryWithAssoc>> {
...
pub fn find_most_recent(
    titan_db: &MysqlConnection
) -> db::TitanQueryResult<models::UserFileEntry> {
...
pub fn create_file_entry(
    ...
    wcf_db: &MysqlConnection,
    app_config: &State<config::AppConfig>
) -> db::TitanQueryResult<models::UserFileEntryWithAssoc> {
...
pub fn map_file_entry_assoc(
    ...
    let user_profile =
        accounts::users::find_by_id(file_entry.user_id, titan_db)
            .and_then(|user| accounts::users::map_user_to_profile(
    ...
    let modified_by_profile =
        accounts::users::find_by_id(file_entry.modified_by, titan_db)
            .and_then(|user| accounts::users::map_user_to_profile(
                &user, wcf_db, app_config))?;
